...
def download_and_analyze(lat, lon):
    ...
    filter_query = (
        f"$filter=OData.CSC.Intersects(area=geography'SRID=4326;POINT({lon} {lat})') "
        "and contains(Name,'S1A_IW_GRDH')"
    )

    try:
        res = requests.get(
            f"{base_url}?{filter_query}&$top=1&$orderby=ContentDate/Start desc",
            headers=headers,
        )
        ...
        ql_url = (
            ...
            f"({p_id})/Nodes('{name}')/Nodes('QuickLook')/$value"
        )
        ...
        if ql_res.status_code != 200:
            print("--- QuickLook node failed, trying Thumbnail node ---")
            ql_url = (
                "https://catalogue.dataspace.copernicus.eu/odata/v1/Products"
                f"({p_id})/Nodes('{name}')/Nodes('Thumbnail')/$value"
            )
            ql_res = requests.get(ql_url, headers=headers, stream=True, timeout=60)
